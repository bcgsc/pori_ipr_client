class VariantCountsService {
  ...
  async get(POGID, report) {
    const resp = await this.$http.get(`${this.api}/${POGID}/report/${report}/genomic/summary/variantCounts`);
    return resp.data;
  }
  ...
  async update(POGID, report, analysis) {
    const resp = await this.$http.put(`${this.api}/${POGID}/report/${report}/genomic/summary/variantCounts/`, analysis);
    return resp.data;
  }
}
