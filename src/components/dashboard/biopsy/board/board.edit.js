      ...
      $scope.save = () => {
        ...
        $analysis.update(analysis)
          .then((result) => {
            ...
            const oldProjects = analysis.pog.projects;

            // unbind from projects no longer in list
            const unbind = _.differenceBy(oldProjects, $scope.patient.projects, 'ident');
            _.each(unbind, (project) => {
              $project.pog(project.ident).remove(analysis.pog.ident);
            });

            // bind to new projects in list
            const bind = _.differenceBy($scope.patient.projects, oldProjects, 'ident');
            _.each(bind, (project) => {
              $project.pog(project.ident).add(analysis.pog.ident);
            });
          ...
          .catch((err) => {
            $mdToast.showSimple(`An error occurred and the biopsy was not updated: ${err}`);
          });
      };
    }]);
