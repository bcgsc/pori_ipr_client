class CopyNumberAnalyses {
  ...
  async all(report) {
    const { data } = await this.$http.get(
      `${this.api}/${report}/genomic/copy-number-analyses/cnv`,
    );
    return data;
  }
  ...
  async updateOne(report, ident, payload) {
    const { data } = await this.$http.put(
      `${this.api}/${report}/genomic/copy-number-analyses/cnv/${ident}`,
      payload,
    );
    return data;
  }
  ...
  async getType(report, type) {
    const { data } = await this.$http.get(
      `${this.api}/${report}/genomic/copy-number-analyses/cnv/${type}`,
    );
    return data;
  }
}
