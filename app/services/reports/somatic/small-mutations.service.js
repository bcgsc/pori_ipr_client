class SmallMutationsService {
  ...
  async all(report) {
    const { data } = await this.$http.get(
      `${this.api}/${report}/genomic/somatic-mutations/small-mutations`,
    );
    return data;
  }
  ...
  async update(report, ident, payload) {
    const { data } = await this.$http.put(
      `${this.api}/${report}/genomic/somatic-mutations/small-mutations/${ident}`,
      payload,
    );
    return data;
  }
  ...
  async getType(report, type) {
    const { data } = await this.$http.get(
      `${this.api}/${report}/genomic/somatic-mutations/small-mutations/${type}`,
    );
    return data;
  }
}
